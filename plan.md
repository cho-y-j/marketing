...
### 3-1. 기본 체력 판단
- [ ] 리뷰 수 기준 우선순위 로직 (일정 수준 이하 → 리뷰 확보 우선)
- [ ] 판단 결과 → PendingAction 또는 대시보드 액션으로 연결

### 3-2. 트래픽 판단
- [ ] 리뷰 있는데 검색 유입 없는 경우 탐지
- [ ] 트래픽 유입 전략 추천 로직

### 3-3. 업종별 키워드 전략 분기
- [ ] KeywordRule 기반 업종별 전략 자동 변경
- [ ] 키워드 우선순위 자동 산정

### 3-4. "오늘의 액션" 엔진
- [ ] Phase 3-1~3-3 로직 종합 → 매일 3가지 실행 액션 자동 생성
- [ ] 대시보드 액션 카드와 연결
...
### 4-1. 블로그 노출 체크 (CLI)
- [ ] 키워드별 네이버 검색 → 블로그 영역 노출 여부 확인
- [ ] 상위 블로그 문서 수집 (제목, 작성일, 블로그명)
- [ ] 반복 노출 블로그 탐지
- [ ] 경쟁 강도 분석 (블로그 수, 최근 30일 작성 비율)
...
- [ ] 홈 (오늘 해야 할 것)
- [ ] 내 매장 상태 (매장 분석 + 경쟁 비교)
- [ ] 마케팅 실행 (키워드 + 콘텐츠 + 리뷰 관리)
- [ ] 성과 확인 (리포트)
- [ ] 사이드바 리뉴얼

### 5-2. 외국인 상권 진단
- [ ] "외국인 상권이세요?" 체크 UI
- [ ] 안내 메시지 (구글 지도+검색+리뷰 구조)
- [ ] CTA 버튼 3종
- [ ] CTA 클릭 → 매장 정보 폼 → 진단 결과

### 5-3. 전문 상담 CTA
- [ ] 부족점 진단 결과에 "전문 상담 받기" CTA 연결
- [ ] 상담 신청 폼 (카카오톡/전화 연결)
- [ ] 관리자 화면에서 상담 신청 목록 확인

### 5-4. 생성형 AI 작업 (API Key 기반)
- [ ] 고객 API Key 저장/암호화 구조 (이미 있음 - anthropicApiKey)
- [ ] AI 작업 시 고객 키 우선 사용 로직
- [ ] 블로그 글 생성 (고객 키)
- [ ] 리뷰 댓글 생성 (고객 키)
- [ ] 키 없을 때 서비스 키 폴백 또는 제한 안내
